...
- Migration rules: the 26.1-era `minecraft:sum` number provider (`summands`) converts to the 26.3 `minecraft:add` provider (`inputs`) and back, gated on the provider-only field so the same-named density functions and enchantment level-based values are never rewritten; a pack still shipping the pre-26.3 `number_provider` registry now fails closed with `number-provider-registry-split`.
- Migration rules: the decorated-pot block-entity `sherds` field converts between the legacy four-entry list and the 26.3 face map inside `data merge block` commands and structure `blocks[].nbt`, matching the item component. `product`/`minimum`/`maximum`/`average` needed no migration: they only ever existed in 26.3 snapshots.
...
- TUI: the plugin list is now built in a worker thread, so installing a plugin whose import is slow no longer freezes the interface on every render or language switch; the marketplace detail page reuses the installed set the list already computed instead of re-inspecting the store on each render.
