...
完整插件开发说明见 plugin-development/PLUGIN_DEVELOPMENT.zh-CN.md。
"""
...
- `{name}.example@88`
""",
    # 可选：多语言显示。键为语言代码（与 TUI 的语言一致，如 "en"），
    # TUI 会跟随当前界面语言自动切换插件的名称、简介与文档预览。
    "localizations": {{
        "en": {{
            "name": "{name}",
            "description": "Describe which migrations this plugin performs.",
            "readme": """# {name}

**Plugin id**: `{name}@88`
**Target version**: `1.21.9`

## What this plugin does

Explain the migration in Markdown: use cases, boundaries, and safety limits.
""",
        }}
    }},
    "official_sources": [
...
- `PLUGIN["localizations"]` 可选：为不同语言提供 `name`/`description`/`readme`，TUI 预览会跟随界面语言自动切换。
...
- 开发前请阅读 `plugin-development/PLUGIN_DEVELOPMENT.zh-CN.md` 与 `docs/RULE_AUTHORING.zh-CN.md`。
- 规则 id 全局唯一；规则必须有一手来源；不可证明等价时失败关闭。
"""


class PluginLocalization(FrozenModel):
    """Translated plugin display strings for one language code.

    ``name`` and ``description`` mirror the top-level fields in meaning; ``readme`` is
    the Markdown documentation in that language and falls back to the top-level readme
    when empty.  Language codes follow the TUI language registry (``dpcompat.i18n``).
    """
    ...
    def trimmed(cls, value: str) -> str:
        if value != value.strip():
            raise ValueError("Localized plugin name and description must be trimmed")
        return value
...
class PluginMeta(FrozenModel):
    ...
    readme: str = ""
    # Optional per-language display strings; the TUI picks the entry matching its
    # current language and falls back to the top-level fields otherwise.
    localizations: dict[str, PluginLocalization] = Field(default_factory=dict)
...
class PluginInfo(FrozenModel):
    ...
    def localized(self, language: str) -> PluginInfo:
        """Return a copy whose display fields match ``language`` when available.

        The top-level fields are the author's canonical language and always work as a
        fallback; a localization entry with an empty ``readme`` keeps the canonical one.
        """

        entry = self.localizations.get(language)
        if entry is None:
            return self
        return self.model_copy(
            update={
                ...
                "readme": entry.readme or self.readme,
            }
        )
...
_README_STRINGS: dict[str, dict[str, str]] = {
    "id": {"zh-CN": "插件 id", "en": "Plugin id"},
    "target": {"zh-CN": "目标版本", "en": "Target version"},
    "rules": {"zh-CN": "规则数", "en": "Rule count"},
    "about": {"zh-CN": "说明", "en": "About"},
    "rules_title": {"zh-CN": "包含的规则", "en": "Included rules"},
    "footer": {
        "zh-CN": (
            '本文档由 DPCompat 根据内置插件元数据自动生成；自定义插件可以在 `PLUGIN["readme"]` 中提供自己的完整说明。'
        ),
        "en": "This document is generated by DPCompat from the built-in plugin metadata; custom "
        'plugins can provide their own documentation in `PLUGIN["readme"]`.',
    },
}
...
def _builtin_readme(
    ...
    *,
    ...
    Built-ins ship no hand-written readme, so the detail page is generated from the
    catalog metadata: identity, purpose, and the rules it owns.  ``language`` selects
    the localized heading strings; the name and description must already be localized
    by the caller.
    """

    def field(label: str) -> str:
        entry = _README_STRINGS[label]
        return entry.get(language) or entry.get("zh-CN") or label

    lines = [
        f"# {name}",
        "",
        f"**{field('id')}**：`{plugin_id}`  ",
        f"**{field('target')}**：`{target_version}`  ",
        f"**{field('rules')}**：{len(rule_ids)}",
        "",
        f"## {field('about')}",
        "",
        description,
...
# English display strings for the built-in catalog.  The canonical (zh-CN) name and
# description live in _BUILTIN_PLUGIN_DEFS; each entry here carries the "en" variant.
_BUILTIN_PLUGIN_L10N: dict[str, dict[str, tuple[str, str]]] = {
    "text-components@71": {
        "en": (
            "Text component event migration",
            "Migrate pre-1.21.5 clickEvent/hoverEvent fields to snake_case and action-specific "
            "fields across known JSON text locations and SNBT components inside commands.",
        )
    },
    "item-components@71": {
        "en": (
            "Item tooltip component migration",
            "Consolidate per-component show_in_tooltip flags into minecraft:tooltip_display and "
            "convert officially simplified component shapes.",
        )
    },
    "entity-nbt@71": {
        "en": (
            "Entity NBT field migration",
            "Migrate entity NBT in summon and data merge entity: equipment slot merging, "
            "fall_distance, sleeping coordinates, item_frame/phantom/player fields, and embedded "
            "text components such as CustomName.",
        )
    },
    "structure-nbt@71": {
        "en": (
            "Structure file entity migration",
            "Rewrite 1.21.5 entity fields inside data/<namespace>/structure/*.nbt entities[].nbt "
            "while preserving binary NBT types.",
        )
    },
    "command-slots@71": {
        "en": (
            "Saddle slot rename",
            "Rename the horse.saddle slot to saddle in item commands; never rewrites NBT paths in data commands.",
        )
    },
    "strict-json@80": {
        "en": (
            "Strict JSON normalization",
            "Rewrite every JSON resource as standard strict JSON for 1.21.6+ targets; duplicate "
    ...
    "identifiers@88": {
        "en": (
            "Identifier renames",
            "Rename exact minecraft:chain identifiers to minecraft:iron_chain in JSON scalars and "
            "complete command tokens only, never object keys or substrings.",
        )
    },
    "spawn-rotation@88": {
        "en": (
            "Spawn point rotation syntax",
            "Append yaw/pitch to spawnpoint/setworldspawn; downgrade is lossless only when the pitch is zero.",
        )
    },
    "gamerules@94.1": {
        "en": (
            "Gamerule registry renames",
            "Migrate 1.21.11 gamerules to minecraft:snake_case, handling special renames and "
            "inverted rules with explicit value flips; removed fire rules block both ways.",
        )
    },
    "worldborder@94.1": {
        "en": (
            "Worldborder time units",
            "Convert seconds/days/ticks between the time units, but real-time vs game-tick "
            "progression is not equivalent, so timed commands default to unknown.",
        )
    },
    "loot@94.1": {
        "en": (
            "Filtered loot branches",
            "Rename the filtered loot function's modifier to on_pass; downgrade is blocked when on_fail is present.",
        )
    },
    "clocks@101.1": {
        "en": (
            "World clock defaults",
            "Add or remove the vanilla overworld clock default for timeline, test_environment, and "
            "time_check; custom clocks and time markers are blocked.",
        )
    },
    "recipes@101.1": {
        "en": (
...
def _builtin_plugins() -> tuple[PluginInfo, ...]:
    ...
    for plugin_id, name, description, rule_ids, target_version in _BUILTIN_PLUGIN_DEFS:
        ...
        localizations: dict[str, PluginLocalization] = {}
        for language, (localized_name, localized_description) in _BUILTIN_PLUGIN_L10N.get(plugin_id, {}).items():
            localizations[language] = PluginLocalization(
                name=localized_name,
                description=localized_description,
                readme=_builtin_readme(
                    plugin_id,
                    localized_name,
